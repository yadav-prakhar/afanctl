...
- [ ] f. (user-gated) `afanctl curve` + 1-hour soak; then `doctor --compare 600` for the empirical record. **Hold until T9c clears** (round-3 review of F19/F20 in flight), then: `sudo afanctl curve`, confirm `fan1_manual` reads `1` and the rpm tracks the curve, soak an hour watching `status`, then `sudo afanctl observe` and run `doctor --compare 600` in observe for the SMC-vs-ours table.
