...
**Read the bound ABI generation first.** The sysfs attribute names and the AUTO token are generation-dependent (#2, ruling F26). Establish the bound generation by *probing*, never by parsing a version string — `doctor` prints it and `detect_abi` does the same:

| | Legacy (kernel ≤ 7.2) | Modern (kernel ≥ 7.3) |
|---|---|---|
| mode attribute | `fan1_manual` | `pwm1_enable` |
| rpm attribute | `fan1_output` | `fan1_target` |
| AUTO token | `0` | `2` (Manual = 1; `0` is rejected `-EINVAL`) |

Every bare `fan1_manual` / `fan1_output` / `0` below means **the mode attribute, rpm attribute and AUTO token of the bound generation**. The 2026-09-14 transcript was recorded on the legacy generation (A1708, kernel ≤ 7.2) and is kept below; a **2026-09-24 re-run re-discharged the gate generation-explicitly on that same legacy binding**, after F26/F27 landed in `dev`.
...
- [x] a. `sudo afanctl doctor` — **PASS 2026-09-14** (7 PASS + 1 expected WARN: unit not installed). **PASS 2026-09-24 (legacy)**: all PASS, no WARN, and the new generation line reads `applesmc ABI generation — legacy (kernel <= 7.2): fan1_manual + fan1_output; AUTO restore token "0"`. Do **not** run it as a plain user: the write-mode checks (bound mode attribute writable, L2 fd armed) open the manual file `O_WRONLY`, so they FAIL by design without root (README "Root required (F11)"). `WARN` lines never block; the `systemd unit health — unit not installed` WARN is expected until `makepkg -si` in §9.4.
- [x] b. `sudo afanctl doctor --roundtrip` — **PASS 2026-09-14**: manual @ 1200 rpm verified over 2 s (observed 1210 rpm / Manual during the hold), AUTO restored. **PASS 2026-09-24 (legacy)**: `roundtrip manual write test — manual+1200 rpm verified over 2 s; AUTO restored (observed 2165 rpm / Manual)`; bound mode attribute read `0` afterwards. Non-root it refuses to write (no L2 fd) rather than writing manual — by design.
- [x] c. `sudo afanctl selftest-panic` — **PASS 2026-09-14**: deliberate panic at `src/safety.rs:81`, exit 101; bound mode attribute reads `0` afterwards. **PASS 2026-09-24 (legacy)**: the F27 rewrite moved the sanctioned panic to `src/safety.rs:179`; exit 101, and `fan1_manual` read `0` afterwards — i.e. the death path restored AUTO using the **legacy** token, from the descriptor published by `probe_safe_restore`.
- [x] d. `sudo systemctl start afanctl` (observe) — **PASS 2026-09-14** (soak): the unit ran **1 h 10 min** continuously with `Restart=always`, consumed **1.966 s CPU over 4211 s wall (0.05 %)** and peaked at **2.4 MB** RSS — R11's <0.1 % CPU and <5 MB budget both met, no errors in the journal apart from the deliberate curve experiments. Observed via `systemctl status` + `status --json` reading the state file as a normal user. *(2026-09-24: not re-soaked; the daemon ran throughout the re-run in observe with no errors.)*
- [x] e. `SIGKILL` the daemon in curve mode — **PASS 2026-09-14 21:48** (the acceptance test that had failed three times): `sudo afanctl curve` ⇒ `fan1_manual == 1` within 3 s (first time on hardware), `status` showed `mode: curve`, `manual=true`, `target: 1200 rpm`, fan 2005 rpm converging, `recent_errors: none`; `systemctl kill -s SIGKILL` ⇒ restart at 21:48:46 with the journal showing `startup reconcile: previous process died without restoring AUTO (fan1_manual=1; SIGKILL/OOM-kill cannot run L2); restoring AUTO now` then `startup reconcile: AUTO restored (fan1_manual=0, verified)`, and `fan1_manual` read `0`. `doctor` also PASSed its new stale-binary check (`running daemon matches installed binary`). Note: because `/run/afanctl/cmd.json` holds a standing `curve` command (R8, by design), the restarted daemon re-applies it within a poll, so `manual=1` returns; `sudo afanctl observe` releases the fan. **PASS 2026-09-24 (legacy)**: `curve` ⇒ `fan1_manual == 1` within 1 s, `mode: curve`, `target: 4450 rpm`, `recent_errors: none`; `SIGKILL` (status 9/KILL) ⇒ systemd restarted the unit, journal showed `startup reconcile: previous process died without restoring AUTO (fan mode was Manual at startup; SIGKILL/OOM-kill cannot run L2); restoring AUTO now` then `startup reconcile: AUTO restored (fan mode verified Auto)`, and `fan1_manual` read `0`. Startup line: `mode=observe, backend=applesmc/legacy, l2_armed=true, watchdog_notify=true`. *(Unlike 2026-09-14 the restarted daemon stayed in observe and did not re-apply the standing curve — `manual` remained `0`.)*
- [x] f. curve soak + `doctor --compare 600` — **PASS 2026-09-14** (ran clean: no errors, no fallback, `status` showed `mode: curve`, `target: 1200 rpm`, `manual=true`; at `t_eff` 63 °C — below `high = 66` — our curve held 1200 rpm, fan 1174). **Empirical verdict from the 600-sample comparison: ours does not beat the incumbent at idle** — mean Δ 191 rpm, max |Δ| 2260, ours quieter in 198 / louder in 392 / equal in 10. Read by regime: at `t_eff` ≤ 64 °C ours ≈ the SMC within ±20 rpm (both ~1190–1220); during short coretemp spikes (66–85 °C) ours ramps toward the linear target at the 750 rpm/poll slew while the SMC stayed near 1.2k rpm (its own sensors did not register those spikes) — hence the loud-side deltas up to ~2.2k rpm. So the honest characterisation: ours is **more responsive to coretemp and verifiable**, not quieter. Knobs if quiet is the goal: raise `[thresholds].high` (start ramping later) and/or lower `[curve].max_rpm`. **2026-09-24 (legacy, shortened to `--compare 60`)**: ran clean (no errors, `mode: curve`, `target` ramping); verdict `mixed: quieter in 32/60, louder in 28/60 (mean Δ -103 rpm)` — consistent with the 2026-09-14 characterisation. The full 600-sample comparison was not re-run.
- [x] g. `hold 3000` via the polkit rule — **PASS 2026-09-14**: `pkexec afanctl hold 3000` (passwordless rule worked, no prompt), `status` → `mode: hold`, `target: 3000 rpm`, fan **2980 rpm**, `manual=true`, `recent_errors: none`; `sudo afanctl observe` released it. **PASS 2026-09-24 (legacy)**: `pkexec afanctl hold 3000` returned 0 with no prompt, `mode: hold`, `target: 3000 rpm`, fan 2939–3000 rpm, `manual=true`; `recent_errors` listed the earlier reconcile event, not a hold fault. **Two corrections to this step as originally written:** (i) `once --at-temp 86 --dry-run` prints `SetSpeed(1950)`, not `EscalateMax` — the decision carries the slew-limited step (750 rpm from the previous command) and the overshoot guard requires `OVERSHOOT_POLLS = 3` **consecutive polls inside one process**, while `once` is stateless, so the guard is unreachable from the CLI and is proven by the policy trace tests (PRD §6.R10) instead; (ii) `doctor` did not warn "hold active" — that check simply did not exist (ticket F25 adds it). PRD §9.3g corrected accordingly.
- [x] `makepkg -si` builds and installs cleanly; `systemctl enable` survives a reboot test (boots in observe, fan on the SMC curve). **PASS 2026-09-15 02:17 (post-reboot, verified): unit enabled AND active since boot, mode observe, `fan1_manual` = 0, fan ~1.2k rpm on the SMC curve, doctor all PASS, NRestarts = 0. The whole §9.3/§9.4 gate is discharged.** **2026-09-24 (legacy)**: `makepkg -sif` from the dev working tree reinstalled cleanly; unit restarted and `doctor`'s stale-binary check PASSed (`running daemon matches installed binary`). The reboot test was not re-run.
