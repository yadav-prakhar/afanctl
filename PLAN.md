...
- [ ] e. `SIGKILL` the daemon in curve mode → it restarts within ~1 s and the **startup reconcile** restores AUTO → daemon resumes. **Run only after F16 lands**: pre-F16 `curve` self-disables to monitor-only and restores AUTO (F16), so the daemon would not be in manual mode when killed and the test would prove nothing. (The F14 fix itself is already installed and verified in code/tests.) Verify via `status` + journal + `cat /sys/devices/platform/applesmc.768/fan1_manual`.
- [ ] f. (user-gated) `afanctl curve` + 1-hour soak; then `doctor --compare 600` for the empirical record. **BLOCKED until F16 lands** (curve mode self-disables on real hardware: write-verify compared the tach against the command, and L1 counted fan deceleration as a write failure — see the ledger). After F16: reinstall, `sudo afanctl curve`, confirm `fan1_manual` reads `1` and the rpm actually tracks the curve.
